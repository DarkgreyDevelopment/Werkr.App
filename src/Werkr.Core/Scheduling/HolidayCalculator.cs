...
public static class HolidayCalculator {
    ...
    /// </summary>
    public static IReadOnlyList<HolidayDate> ComputeDatesForYear(
        HolidayRule rule,
        int year
    ) {
        // Year bounds check
        ...
        DateOnly? rawDate = rule.RuleType switch {
            HolidayRuleType.FixedDate => ComputeFixedDate(
                rule,
                year
            ),
            HolidayRuleType.NthWeekdayOfMonth => ComputeNthWeekday(
                rule,
                year
            ),
    ...
    /// </summary>
    public static IReadOnlyList<HolidayDate> ComputeAllDatesForYear(
        HolidayCalendar calendar,
        int year
    ) {
        List<HolidayDate> results = [];
        foreach (HolidayRule rule in calendar.Rules) {
            results.AddRange( ComputeDatesForYear(
                rule,
                year
            ) );
        }
    ...
    /// </summary>
    public static IReadOnlyList<HolidayDate> ComputeDatesForRange(
        HolidayCalendar calendar,
        int startYear,
        int endYear
    ) {
        ...
        for (int year = startYear; year <= endYear; year++) {
            results.AddRange( ComputeAllDatesForYear(
                calendar,
                year
            ) );
        }
    ...
    /// </summary>
    internal static DateOnly ApplyObservanceRule(
        DateOnly date,
        ObservanceRule rule
    ) => rule switch {
    ...
    /// </summary>
    internal static DateOnly GetLastWeekdayOfMonth(
        int year,
        int month,
        DayOfWeek day
    ) {
    ...
    private static DateOnly? ComputeFixedDate(
        HolidayRule rule,
        int year
    ) {
        ...
        return day > DateTime.DaysInMonth(
            year,
            month
        ) ? null : new DateOnly(
            year,
    ...
    private static DateOnly? ComputeNthWeekday(
        HolidayRule rule,
        int year
    ) {
        return !rule.Month.HasValue || !rule.DayOfWeek.HasValue || !rule.WeekNumber.HasValue
            ...
            : GetNthWeekdayOfMonth(
                ...
                rule.WeekNumber.Value
            );
    }

    /// <summary>Last weekday of month.</summary>
    private static DateOnly? ComputeLastWeekday(
        HolidayRule rule,
        int year
    ) {
