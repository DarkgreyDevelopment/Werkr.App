...
public class RegistrationBundleGeneratorTests {
    ...
    public void CreateBundle_DefaultSettings_ProducesValidEntity( ) {
        (string encrypted, RegistrationBundle entity) = RegistrationBundleGenerator.CreateBundle(
            "TestConn", "https://server:5000", "password123" );

        Assert.IsNotNull( encrypted );
        Assert.IsNotNull( entity );
        Assert.AreEqual( "TestConn", entity.ConnectionName );
        Assert.HasCount( 16, entity.BundleId );
        Assert.AreEqual( RegistrationStatus.Pending, entity.Status );
        Assert.AreEqual( 4096, entity.KeySize );
    }
    ...
    public void CreateBundle_DefaultExpiration_ExpiresIn24Hours( ) {
        ...
        (_, RegistrationBundle entity) = RegistrationBundleGenerator.CreateBundle(
            "Conn", "https://server", "pass" );

        DateTime after = DateTime.UtcNow.AddHours( 24 );

        // MSTest v4: IsGreaterThanOrEqualTo(lowerBound, value) asserts value >= lowerBound
        Assert.IsGreaterThanOrEqualTo( before, entity.ExpiresAt );
        Assert.IsLessThanOrEqualTo( after, entity.ExpiresAt );
    }
    ...
    public void CreateBundle_CustomExpiration_SetsCorrectExpiry( ) {
        ...
        (_, RegistrationBundle entity) = RegistrationBundleGenerator.CreateBundle(
            "Conn", "https://server", "pass", expiration: customExpiration );
    ...
    public void CreateBundle_EncryptedBundle_DecryptableWithPassword( ) {
        ...
        (string encrypted, RegistrationBundle entity) = RegistrationBundleGenerator.CreateBundle(
            "TestConn", "https://server:5000", password );

        RegistrationBundlePayload payload = RegistrationBundlePayload.FromEncryptedString( encrypted, password );

        CollectionAssert.AreEqual( entity.BundleId, payload.BundleId );
        Assert.AreEqual( "TestConn", payload.ConnectionName );
        Assert.AreEqual( "https://server:5000", payload.ServerUrl );
    }
}
