...
public sealed class CopyFileHandler : IActionHandler {
    ...
    public CopyFileHandler( IFilePathResolver resolver, ILogger<CopyFileHandler> logger ) {
        _resolver = resolver;
        _logger = logger;
    }
    ...
    public async Task<ActionOperatorResult> ExecuteAsync(
        ...
        try {
            ...
            if (Directory.Exists( source )) {
                ...
                if (result) {
                    await output.WriteAsync(
                        OperatorOutput.Create( LogLevel.Information, $"Copied directory '{source}' → '{destination}'" ),
                        cancellationToken );
                } else {
                    await output.WriteAsync(
                        OperatorOutput.Create( LogLevel.Warning, $"Source directory '{source}' does not exist." ),
                        cancellationToken );
                ...
                foreach (string file in files) {
                    ...
                    string dest = Directory.Exists( destination )
                        ...
                        : destination;
                    File.Copy( file, dest, p.Overwrite );
                    await output.WriteAsync(
                        OperatorOutput.Create( LogLevel.Information, $"Copied '{file}' → '{dest}'" ),
                        cancellationToken );
                }
            }
            ...
            _logger.LogError( ex, "CopyFile action failed" );
            await output.WriteAsync(
                OperatorOutput.Create( LogLevel.Error, $"CopyFile failed: {ex.Message}" ),
                cancellationToken );
            return new ActionOperatorResult( Success: false, Exception: ex );
        }
    }

    private static bool CopyDirectory( string source, string destination, bool overwrite, bool recursive ) {
        ...
        if (recursive) {
            foreach (DirectoryInfo subDir in subDirs) {
                ...
                _ = CopyDirectory( subDir.FullName, newDest, overwrite, recursive );
            }
        } else {
            foreach (DirectoryInfo subDir in subDirs) {
                _ = Directory.CreateDirectory( Path.Join( destination, subDir.Name ) );
            }
        }
