...
/// <summary>
/// Agent-local equivalent of <see cref="Werkr.Core.Tasks.JobOutputWriter"/>.
/// Writes job output to individual log files on the Agent's disk. Each job gets
/// a file named <c>{JobId}.log</c> in the configured output directory. Output is
/// appended incrementally so partial output is preserved if the job crashes.
/// </summary>
...
public sealed class AgentJobOutputWriter( IOptions<JobOutputOptions> options, ILogger<AgentJobOutputWriter> logger ) {
    ...
    public void EnsureDirectoryExists( ) {
        if (!Directory.Exists( _outputDirectory )) {
            ...
            if (logger.IsEnabled( LogLevel.Information )) {
                logger.LogInformation( "Created agent job output directory: {Directory}", _outputDirectory );
            }
        }
    }
    ...
    public string GetOutputPath( Guid jobId ) =>
        Path.Combine( _outputDirectory, $"{jobId}.log" );

    /// <summary>
    /// Returns the relative file name for a job's output log (for storing in <c>OutputPath</c>).
    /// </summary>
    /// <param name="jobId">The job identifier.</param>
    /// <returns>Relative file name.</returns>
    public static string GetRelativeOutputPath( Guid jobId ) =>
        $"{jobId}.log";
    ...
    public async Task WriteLineAsync( Guid jobId, OperatorOutput output, CancellationToken ct = default ) {
    ...
    /// <param name="ct">Cancellation token.</param>
    public async Task WriteLinesAsync( Guid jobId, IEnumerable<OperatorOutput> outputs, CancellationToken ct = default ) {
        EnsureDirectoryExists( );
        string filePath = GetOutputPath( jobId );
        IEnumerable<string> lines = outputs.Select( FormatLine );
    ...
    public async Task<string?> ReadFullOutputAsync( Guid jobId, CancellationToken ct = default ) {
        string filePath = GetOutputPath( jobId );
    ...
    /// <param name="jobId">The job identifier.</param>
    /// <param name="ct">Cancellation token.</param>
    /// <returns>Tail preview string, or null if no output exists.</returns>
    public async Task<string?> GetTailPreviewAsync( Guid jobId, CancellationToken ct = default ) {
        string filePath = GetOutputPath( jobId );
